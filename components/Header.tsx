  ...
  return (
    <nav className={`flex justify-between items-center px-20 max-sm:px-10 w-screen h-24 absolute`} style={{ background: `${pathname === "/" ? "#FFE5E538": "#FFE5E5"}`}}>
      ...
      <div className="flex justify-between ">
        {
          ...
          navLinks.map((link) => {
            ...
            return (
              <Link key={link.label} className={`pr-8 max-sm:hidden ${pathname === "/" ? `${isActive && 'text-light-1'}` : `${isActive ? 'text-[#756AB6]' : 'text-[#E0AED0]'}`} `} href={link.route }>{link.label}</Link>
            )
          })}
      ...
      > 
          ...
          navLinks.map((link) => {
            return (
              <Link key={link.label} href={link.route}><MenuItem onClick={handleClose}>{link.label}</MenuItem></Link>
            )
          })}
      </Menu>
    </div>
